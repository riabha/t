    ...
    for ti, task in enumerate(tasks):
        ...
        if task["theory_credits"] > 0:
            for d in DAYS:
                ...
                # Strict mode only applies when the section opted in for morning labs
                strict_lab_timing = False      # default — always initialized
                preferred_start_slot = None    # default — always initialized
                task_wants_morning_here = (
                    morning_lab_section_ids is None  # None = fallback mode: all sections
                    or bool(set(morning_lab_section_ids) & set(task["section_ids"]))
                )
                if task_wants_morning_here:
                    for lr in lab_rules:
        ...
        if task["lab_credits"] > 0:
            for d in DAYS:
                ...
                # ── Determine if morning is preferred for this specific section ──
                # morning_lab_section_ids controls which sections get morning labs.
                # None  = fallback mode: ALL sections allowed morning labs
                # []    = no morning labs for anyone
                # [ids] = only those sections
                task_wants_morning = (
                    morning_lab_section_ids is None
                    or bool(set(morning_lab_section_ids) & set(task["section_ids"]))
                )
                lab_morning_pref = (d in task["lab_morning_days"]) or task_wants_morning
                ...
                for lr in lab_rules:
                    if (not lr.get("dept") or task["dept_code"] == lr["dept"]) and \
                       (not lr.get("batch") or task["batch_year"] == lr["batch"]):
                        ...
                        # Strict mode only activates if section is marked for morning
                        if task_wants_morning:
                            preferred_start_slot = lr.get("preferred_start_slot")
                            strict_lab_timing = lr.get("strict_mode", False)
                        break
