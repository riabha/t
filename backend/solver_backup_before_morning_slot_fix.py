...
def generate_timetable(db: Session, name: str = "Auto Generated",
            ...
            for d in DAYS:
                ...
                if batch_config:
                    ...
                    if mode == "strict" and is_morning_day:
                        ...
                        allowed_lab_starts = [0] if 0 in lab_starts[d] else []
                    else:
    ...
    for sec_id, t_indices in section_tasks.items():
        ...
        for d in DAYS:
            ...
            # No-Gaps Constraint: If s1 and s3 are occupied, s2 MUST be occupied
            # HARD constraint when no_gaps=True AND strict mode is NOT active
            # SOFT constraint when no_gaps=False OR strict mode IS active
            # (Strict mode needs flexibility for shared teachers across batches)
            if len(slots) > 2:
                for i in range(len(slots)):
                    for j in range(i + 2, len(slots)):
                        ...
                        if s1 in occupied and s3 in occupied:
                            for s2 in s2_list:
                                if s2 in occupied:
                                    # Make no-gaps SOFT when strict mode is active
                                    if section_no_gaps and not is_strict_day:
                                        # HARD CONSTRAINT: No gaps allowed
                                        # If s1 and s3 are occupied, s2 MUST be occupied
                                        # occupied[s1] + occupied[s3] <= 1 + occupied[s2]
                                        model.Add(occupied[s1] + occupied[s3] <= 1 + occupied[s2])
                                    else:
                                        # SOFT CONSTRAINT: Gaps discouraged but allowed
        ...
        for sec_id, t_indices in section_tasks.items():
            ...
            if len(days_with_end_times) >= 2:
                ...
                for d in days_with_end_times:
                    ...
                    # Apply EXTREME penalty for deviation from average
                    # Penalty = |deviation| * 10,000,000
                    # This MUST override ALL other constraints to force balanced end times
