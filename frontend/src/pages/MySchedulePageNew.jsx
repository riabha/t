...
export default function MySchedulePage() {
    ...
    const loadAllSchedules = async (timetablesToLoad) => {
        try {
            const teacherName = user?.full_name || '';
            console.log('Loading schedules for teacher:', teacherName);
            console.log('Timetables to load:', timetablesToLoad.length);
            let allSlots = [];

            // Load slots from ALL timetables to capture cross-department teaching
            for (const tt of timetablesToLoad) {
                console.log(`Loading timetable ${tt.id}: ${tt.name}`);
                const res = await api.get(`/timetable/${tt.id}`);
                console.log(`  Total slots in timetable: ${res.data.slots?.length || 0}`);

                // Filter slots based on role
                const slots = (res.data.slots || []).filter(s => {
                    // For lab slots: only show if user is the lab engineer
                    // For theory slots: only show if user is the teacher
                    if (s.is_lab) {
                        return s.lab_engineer_name === teacherName;
                    } else {
                        return s.teacher_name === teacherName;
                    }
                });

                console.log(`  Slots for ${teacherName}: ${slots.length}`);
            ...
            console.log('Total slots across all timetables:', allSlots.length);
            setMySlots(allSlots);
        } catch (e) {
    ...
    const loadSchedule = async (ttId) => {
        ...
        try {
            ...
            // Otherwise show all latest timetables
            const ttToLoad = timetables.find(tt => tt.id === ttId);
            if (ttToLoad) {
                await loadAllSchedules([ttToLoad]);
            }
    ...
    useEffect(() => {
        async function load() {
            try {
                ...
                if (latestTimetables.length > 0 && user?.full_name) {
    ...
    // Compute workload
    const theoryHours = mySlots.filter(s => !s.is_lab && !s.is_break && s.subject_code).length;
    const labHours = mySlots.filter(s => s.is_lab && !s.is_break).length; // Don't multiply by 3 for display
    ...
    const exportToPDF = () => {
        try {
            ...
            Object.entries(subjectDetails).forEach(([code, details]) => {
                // Calculate contact hours: theory + lab (no multiplication)
                const theoryHours = details.theoryCredits || 0;
                const labHours = details.labCredits || 0; // Don't multiply by 3
    ...
    const exportToICal = () => {
        ...
        mySlots.forEach(slot => {
            ...
            // Calculate time (assuming 8:30 AM start, 60 min slots)
            const startHour = 8;
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    <button
                        onClick={async () => {
                            setActiveTT(null);
                            setLoading(true);
                            await loadAllSchedules(timetables.filter(tt => tt.status !== 'archived'));
                            setLoading(false);
                        }}
                    ...
                    {timetables.map(tt => (
                        <button key={tt.id} onClick={() => loadSchedule(tt.id)}
                            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${activeTT === tt.id
                                ? 'bg-primary-500 text-white shadow-md shadow-primary-500/20'
                                : 'bg-slate-100 text-slate-500 hover:text-slate-800'
                                }`}>
                            {tt.name}
